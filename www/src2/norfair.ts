...
function multiplyMatrices(A: Matrix, B: Matrix): Matrix {
  ...
  if (colsA !== rowsB) {
    throw new Error(
      `Matrix multiplication dimension mismatch: ${colsA} !== ${rowsB}`,
    );
  }
...
function multiplyMatrixVector(A: Matrix, x: Vector): Vector {
  ...
  if (colsA !== x.length) {
    throw new Error(
      `Matrix-vector multiplication dimension mismatch: ${colsA} !== ${x.length}`,
    );
  }
...
export class TrackedObject {
  ...
  isInitializing(): boolean {
    ...
    if (
      this.isInitializingFlag &&
      this.hitCounter > this.hitInertiaMin + this.initDelay
    ) {
  ...
  hit(detection: Point, period: number = 1): void {
    ...
    if (this.hitCounter < this.hitInertiaMax) {
      ...
      this.hitCounter = Math.min(
        this.hitCounter + 2 * period,
        this.hitInertiaMax,
      );
    }
...
export class Tracker {
  ...
  constructor(options: TrackerOptions) {
    ...
    if (options.initDelay !== undefined && options.initDelay >= 0) {
      ...
      this.initDelay = Math.floor(
        (this.hitInertiaMax - this.hitInertiaMin) / 2,
      );
    }
    ...
    if (this.initialHitCount <= this.hitInertiaMin + this.initDelay) {
      console.warn(
        `initialHitCount (${this.initialHitCount}) should ideally be > hitInertiaMin (${this.hitInertiaMin}) + initDelay (${this.initDelay}) to avoid immediate initialization.`,
      );
    }
  }
  ...
  update(detections: Point[], period: number = 1): number[] {
    ...
    let currentDetections: Detection[] = detections.map((p, i) => ({
      point: p,
      id: i,
    }));
    ...
    const matchedPairsInitialized = this.updateObjectInPlace(
      initializedObjs,
      currentDetections,
    );

    // 6. Match remaining detections to *initializing* objects
    const matchedPairsInitializing = this.updateObjectInPlace(
      initializingObjs,
      currentDetections,
    );
    ...
    const finalResult: number[] = detections.map(
      (_, i) => resultMap.get(i) ?? -1,
    );
  ...
  private updateObjectInPlace(
    ...
    // Returns map: { detection_original_id: object_id }
    ...
    for (const match of distances) {
      ...
      if (
        !matchedDetIndices.has(match.cIdx) &&
        !matchedObjIndices.has(match.oIdx)
      ) {
        // Match found!
