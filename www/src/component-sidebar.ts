...
class ComponentSidebar extends LitElement {
  ...
  #getSelectedCard(): Detection | undefined {
    const det = getDetections().find(
      (d) => d.id === this.#selectedIdController.value,
    );
    if (!det) {
      return null;
    }
  ...
  render() {
    ...
    return html`
      ${detections && detections.length > 0
        ? html` <div class="total-cost">Total Value: $${totalCost}</div> `
        : ""}
      ...
      <div id="card-list">
        ${!detections
          ...
          : detections.map((det) => {
              ...
              return html`
                <div
                  class="detection-item ${det.id === selectedId
                    ? "selected"
                    : ""}"
                  @click=${() => this.#onItemClick(det)}
                >
                  <img
                    src="data:image/jpeg;base64,${det.img}"
                    class="detection-item-img"
                  />
                  <div class="detection-item-info">
                    <span class="detection-item-text"
                      >${match?.name || "Unknown"}
                      ${match?.extra_data?.mana_cost ?? ""}
                      $${match?.all_data?.prices?.usd || "0"}</span
                    >
                    <span class="detection-item-score"
                      >score: ${match?.score || "0"}</span
                    >
                  </div>
                </div>
              `;
      ...
      <div class="all-card-matches">
        ${this.#getSelectedCardMatches().map(
          (match) => html`
            <div class="card-match">${this.renderMatchInfo(match)}</div>
          `,
        )}
      </div>
    `;
  ...
  renderMatchInfo(match: Match) {
    ...
    return html`
      ...
      <p style="font-size: 14px; color: #c0ffee;">
        Match Score: ${match.score}
      </p>
      <p>
        <span style="color: yellow;">Set:</span> ${match.set_name || "Unknown"}
        (${match.set_code || ""})
      </p>
      <p>
        <span style="color: yellow;">Type:</span> ${data?.type_line || "N/A"}
      </p>
      <p>
        <span style="color: yellow;">Price:</span> ${data?.prices?.usd
          ? `$${data.prices.usd}`
          : "N/A"}
      </p>
      ...
      <!--      <img src="data:image/jpeg;base64,${this.#getSelectedCard()
        .img}" alt="${match.name}">-->
    `;
  }
}
